...
public class OperationalHealthService {
    ...
    public ReadinessHealthResponse readiness() {
        List<DependencyHealth> dependencies = checkers.stream()
    ...
    private OperationalStatus aggregate(List<DependencyHealth> dependencies) {
        if (dependencies.stream().anyMatch(dependency -> dependency.status() == OperationalStatus.DOWN)) {
            return OperationalStatus.DOWN;
        }
        if (dependencies.stream().anyMatch(dependency -> dependency.status() == OperationalStatus.DEGRADED)) {
