...
@ConditionalOnProperty(prefix = "qwenbridge.abuse", name = "enabled", havingValue = "true", matchIfMissing = true)
public class AbuseProtectionFilter extends OncePerRequestFilter {
    ...
    private RateLimitDecision evaluate(HttpServletRequest request) {
        ...
        if (isStreamRequest(request) && streamingSessionRegistry.size() >= properties.concurrentStreamLimit()) {
            return RateLimitDecision.rejected(
                    "concurrent-stream",
        ...
        if (apiKey != null && !apiKey.isBlank()) {
            RateLimitDecision apiKeyDecision = rateLimiter.consume(
                    "api-key",
                    fingerprint(apiKey),
    ...
    private boolean isAiRequest(HttpServletRequest request) {
        ...
        return uri.contains("/search/analyze") || uri.contains("/ai/") || uri.contains("/search/stream/");
    }
