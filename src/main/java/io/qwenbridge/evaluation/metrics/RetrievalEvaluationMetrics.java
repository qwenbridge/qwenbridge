...
public class RetrievalEvaluationMetrics {
    ...
    public double precisionAtK(
        ...
        long relevantRetrieved = rankedDocumentIds.stream()
                .limit(k)
                .filter(documentId -> {
                    var label = labels.get(documentId);
                    return label instanceof io.qwenbridge.evaluation.model.RelevanceLabel relevanceLabel
    ...
    public double recallAtK(
        ...
        if (k <= 0 || relevantDocumentCount == 0 || rankedDocumentIds == null || rankedDocumentIds.isEmpty()) {
            return 0.0;
        }
    ...
    public double ndcgAtK(
        ...
        List<String> idealRanking = query.labels()
                .stream()
    ...
    private double dcg(EvaluationQuery query, List<String> documentIds) {
        Map<String, io.qwenbridge.evaluation.model.RelevanceLabel> labels =
                query.labelsByDocumentId();
