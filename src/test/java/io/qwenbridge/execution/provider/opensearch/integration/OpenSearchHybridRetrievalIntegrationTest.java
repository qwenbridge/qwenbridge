...
@EnabledIfEnvironmentVariable(
        named = "QWENBRIDGE_RUN_OPENSEARCH_IT",
        matches = "true"
)
class OpenSearchHybridRetrievalIntegrationTest {
    ...
    void shouldRetrieveRazerFirstForGamingMouseHybridSearch() {
        var embedding = aiService.embed(
                new EmbeddingRequest("gaming mouse razer esports")
        );

        SearchResponse response = openSearchProvider.search(
        ...
        assertThat(firstHit.document().get("title"))
                .isEqualTo("Razer DeathAdder V3");
    }
}
