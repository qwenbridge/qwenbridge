...
public class SearchExampleController {
  ...
  public SearchExampleController(
      QwenBridgeClient client, QwenBridgeStreamingClient streamingClient) {
    this.client = client;
  ...
  public SearchAnalyzeResponse analyze(@RequestParam String query) {
    return client.analyze(SearchAnalyzeRequest.withRequestId(UUID.randomUUID().toString(), query));
  }
