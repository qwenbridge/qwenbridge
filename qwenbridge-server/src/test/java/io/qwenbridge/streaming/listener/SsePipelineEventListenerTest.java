...
class SsePipelineEventListenerTest {
  ...
  void shouldRouteMappedEventToMatchingRequestId() {
    StreamingSessionRegistry registry = mock(StreamingSessionRegistry.class);

    PipelineStreamingEventMapper mapper = new PipelineStreamingEventMapper();

    SsePipelineEventListener listener =
        new SsePipelineEventListener(registry, mapper, new PipelineEventTerminalPolicy());

    PipelineContextSnapshot snapshot =
        new PipelineContextSnapshot(
            "request-1", "desk", false, true, "en", "SEARCH", "ALLOW", 123456789L);

    var event =
        PipelineEvents.stepStarted(
            PipelineStage.INTENT, snapshot, PipelineEventMetadata.of("request-1", 7L));
  ...
  void shouldIgnoreEventsWithoutRequestId() {
    ...
    listener.onPipelineEvent(PipelineEvents.info(PipelineStage.PIPELINE, "hello"));
  ...
  void shouldCompleteMatchingRequestAfterTerminalPipelineEvent() {
    ...
    var event =
        PipelineEvents.pipelineCompleted(snapshot, PipelineEventMetadata.of("request-1", 99L));
  ...
  void shouldNotCompleteRequestAfterNonTerminalStepEvent() {
    ...
    var event =
        PipelineEvents.stepCompleted(
            PipelineStage.SEARCH, snapshot, PipelineEventMetadata.of("request-1", 8L));
  ...
  void shouldPublishStableFailureEventAndCloseRequestWhenPipelineFails() {
    ...
    var event =
        PipelineEvents.pipelineFailed(snapshot, PipelineEventMetadata.of("request-1", 100L));
