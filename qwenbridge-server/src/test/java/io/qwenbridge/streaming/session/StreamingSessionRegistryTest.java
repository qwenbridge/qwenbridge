...
class StreamingSessionRegistryTest {
  ...
  void shouldKeepSessionsSeparatedByRequestId() {
    ...
    StreamingSession unrelated = registry.register("request-2");

    assertThat(registry.findByRequestId("request-1")).containsExactlyInAnyOrder(first, second);

    assertThat(registry.findByRequestId("request-1")).doesNotContain(unrelated);
  ...
  void shouldUpdateLastSeenWhenSessionIsTouched() {
    ...
    assertThat(session.lastSeen()).isAfterOrEqualTo(beforeTouch);
  ...
  void shouldRemoveMatchingSessionsAfterFailureEvent() {
    ...
    registry.failRequest("request-1", "failure-1", "stream.failure", "failure");
