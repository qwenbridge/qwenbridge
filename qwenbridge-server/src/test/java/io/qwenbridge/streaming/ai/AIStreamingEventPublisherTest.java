...
class AIStreamingEventPublisherTest {
  ...
  void shouldPublishTokenEvent() {
    ...
    verify(registry).sendToRequest(eq("request-1"), anyString(), eq("ai.token"), payload.capture());
  ...
  void shouldPublishCompletedEventWithoutClosingSseSession() {
    ...
    verify(registry)
        .sendToRequest(eq("request-1"), anyString(), eq("ai.completed"), payload.capture());
  ...
  void shouldPublishFailedEventWithoutClosingSseSession() {
    ...
    verify(registry)
        .sendToRequest(eq("request-1"), anyString(), eq("ai.failed"), payload.capture());
