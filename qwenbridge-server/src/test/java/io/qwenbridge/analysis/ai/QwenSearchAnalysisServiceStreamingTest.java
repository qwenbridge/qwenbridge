...
class QwenSearchAnalysisServiceStreamingTest {
  ...
  void shouldStreamTokensAndPublishCompletionForRequestAwareAnalysis() {
    ...
    inOrder.verify(streamingEventPublisher).token("request-1", 1L, "{\"intent\":");

    inOrder.verify(streamingEventPublisher).token("request-1", 2L, "\"SEARCH\"}");

    inOrder.verify(streamingEventPublisher).completed("request-1", 2L);
  ...
  void shouldPublishFailureAndReturnFallbackWhenStreamingFails() {
    ...
    verify(streamingEventPublisher)
        .failed("request-1", "AI_STREAM_FAILED", "AI streaming analysis failed");
  ...
  void shouldKeepLegacyNonStreamingBehaviorWithoutRequestId() {
    ...
    when(aiService.chat(any()))
        .thenReturn(new io.qwenbridge.ai.contract.ChatResponse("{\"intent\":\"SEARCH\"}"));
  ...
  void shouldFailAndReturnFallbackWhenTokenLimitIsExceeded() {
    QwenSearchAnalysisService limitedService =
        new QwenSearchAnalysisService(
            ...
            streamingEventPublisher,
            streamingSessionRegistry,
            new StreamingProperties(300_000L, java.time.Duration.ofSeconds(30), 1L, 10L));
    ...
    verify(streamingEventPublisher).token("request-1", 1L, "first");
    verify(streamingEventPublisher)
        .failed("request-1", "AI_STREAM_LIMIT_EXCEEDED", "AI streaming limit exceeded");
    verify(streamingEventPublisher, never()).completed(anyString(), anyLong());
    verify(cache).put(eq(key), any(SearchAnalysis.class));
  }
}
