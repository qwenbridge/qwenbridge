...
class DefaultEvaluationThresholdPolicyTest {
  ...
  private final DefaultEvaluationThresholdPolicy policy = new DefaultEvaluationThresholdPolicy();

  @Test
  void shouldPassWhenAllMetricsMeetThresholds() {
    EvaluationResult result = new EvaluationResult(10, 0.70, 0.80, 0.75, 0.90);
  ...
  void shouldFailWhenAnyMetricIsBelowThreshold() {
    EvaluationResult result = new EvaluationResult(10, 0.59, 0.80, 0.75, 0.90);
  ...
  void shouldFailWithAllViolationsWhenMultipleMetricsAreBelowThreshold() {
    EvaluationResult result = new EvaluationResult(10, 0.10, 0.20, 0.30, 0.40);
  ...
  void shouldFailWhenEvaluationHasNoQueries() {
    EvaluationGateResult gate = policy.evaluate(new EvaluationResult(0, 1.0, 1.0, 1.0, 1.0));
  ...
  void shouldFailWhenMetricIsNanOrInfinite() {
    EvaluationGateResult gate =
        policy.evaluate(new EvaluationResult(1, Double.NaN, Double.POSITIVE_INFINITY, 1.0, 1.0));
