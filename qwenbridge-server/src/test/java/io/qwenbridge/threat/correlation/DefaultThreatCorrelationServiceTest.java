...
class DefaultThreatCorrelationServiceTest {
  ...
  void shouldCorrelatePromptInjectionAndJailbreak() {
    ThreatRiskProfile profile =
        service.correlate(
            List.of(
                finding(ThreatType.PROMPT_INJECTION, 0.78), finding(ThreatType.JAILBREAK, 0.80)));
  ...
  void shouldFallbackToScoreBasedRiskWhenNoCorrelationMatches() {
    ThreatRiskProfile profile = service.correlate(List.of(finding(ThreatType.SQL_INJECTION, 0.85)));
