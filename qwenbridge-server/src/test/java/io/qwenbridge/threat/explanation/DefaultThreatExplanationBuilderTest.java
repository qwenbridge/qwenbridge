...
class DefaultThreatExplanationBuilderTest {
  ...
  private final DefaultThreatExplanationBuilder builder = new DefaultThreatExplanationBuilder();
  ...
  void shouldBuildExplanationFromThreatAnalysis() {
    ...
    ThreatRiskProfile profile =
        new ThreatRiskProfile(0.95, 1.0, ThreatRiskLevel.CRITICAL, List.of(correlation));

    ThreatAnalysis analysis = ThreatAnalysis.from(List.of(finding), ThreatDecision.BLOCK, profile);
    ...
    assertThat(explanation.items().getFirst().message()).isEqualTo("SQL Injection pattern matched");
    assertThat(explanation.items().getFirst().source()).isEqualTo("sql-injection-detector");
    assertThat(explanation.matchedCorrelationRules()).containsExactly("sql_prompt_attack");
  }
