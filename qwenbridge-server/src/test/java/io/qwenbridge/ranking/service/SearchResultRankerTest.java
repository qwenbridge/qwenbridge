...
class SearchResultRankerTest {
  ...
  private final SearchResultRanker ranker = new SearchResultRanker(new DefaultRankingPolicy());
  ...
  void shouldAttachRankingScoreAndSortByFinalScoreDescending() {
    ...
    SearchResultSet ranked = ranker.rank(new SearchResultSet(List.of(first, second), 2, 12));

    assertThat(ranked.totalHits()).isEqualTo(2);
    assertThat(ranked.tookMillis()).isEqualTo(12);
    assertThat(ranked.hits()).extracting(SearchHit::id).containsExactly("doc-2", "doc-1");
    ...
    assertThat(top.metadata().get("rankingScore")).isInstanceOf(RankingScore.class);
  ...
  void shouldPreserveStableOrderByIdWhenFinalScoresTie() {
    ...
    SearchResultSet ranked = ranker.rank(new SearchResultSet(List.of(b, a), 2, 1));

    assertThat(ranked.hits()).extracting(SearchHit::id).containsExactly("doc-a", "doc-b");
  }
