...
class DefaultExecutionEngineTest {
  ...
  private final ExecutionEngine engine =
      new DefaultExecutionEngine(
          List.of(new KeywordSearchExecutor(), new DirectAnswerExecutor()),
  ...
  void shouldExecuteKeywordPlan() {
    ...
    assertThat(result.operations())
        .contains(ExecutionOperation.KEYWORD_SEARCH, ExecutionOperation.RETURN_RESULTS);

    assertThat(result.results()).containsExactly("keyword-search-placeholder-result");
  }
  ...
  void shouldExecuteDirectAnswerPlan() {
    ...
    assertThat(result.operations()).containsExactly(ExecutionOperation.DIRECT_ANSWER);
  }
  ...
  void shouldIgnoreOperationsWithoutRegisteredExecutor() {
    ...
    ExecutionPlan plan =
        ExecutionPlan.builder()
            ...
            .steps(List.of(new ExecutionStep(1, ExecutionOperation.RETURN_RESULTS, "return")))
            .reason("test")
            .build();
