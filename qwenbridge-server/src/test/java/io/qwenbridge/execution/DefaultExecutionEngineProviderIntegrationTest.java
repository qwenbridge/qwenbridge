...
class DefaultExecutionEngineProviderIntegrationTest {
  ...
  void shouldExecuteUsingDefaultSearchProvider() {
    SearchProviderResolver resolver =
        new DefaultSearchProviderResolver(
            new DefaultSearchProviderRegistry(List.of(new InMemorySearchProvider())));
    ...
    ExecutionPlan plan =
        ExecutionPlan.builder()
            ...
            .steps(
                List.of(new ExecutionStep(1, ExecutionOperation.KEYWORD_SEARCH, "keyword search")))
            .reason("keyword search plan")
            .build();

    ExecutionResult result = engine.execute(plan, new ExecutionContext("iphone"));
  ...
  void shouldCreateVectorSearchRequestWithGeneratedEmbedding() {
    ...
    ExecutionPlan plan =
        ExecutionPlan.builder()
            ...
            .steps(List.of(new ExecutionStep(1, ExecutionOperation.VECTOR_SEARCH, "vector search")))
  ...
  void shouldCreateHybridSearchRequestWithGeneratedEmbedding() {
    ...
    ExecutionPlan plan =
        ExecutionPlan.builder()
            ...
            .backend(SearchBackend.OPENSEARCH)
            .steps(List.of(new ExecutionStep(1, ExecutionOperation.HYBRID_SEARCH, "hybrid search")))
  ...
  void shouldRankSearchProviderResultsBeforeStoringResponseInContext() {
    SearchProvider provider = mock(SearchProvider.class);
    when(provider.name()).thenReturn("opensearch");
    when(provider.search(any(SearchRequest.class)))
        .thenReturn(
            new SearchResponse(
    ...
    assertThat(response.results().hits())
  ...
  void shouldApplyRerankingServiceWhenPlanContainsRerankOperation() {
    ...
    io.qwenbridge.reranking.service.RerankingService rerankingService =
        (query, resultSet) ->
            new io.qwenbridge.execution.provider.model.SearchResultSet(
                List.of(resultSet.hits().get(1), resultSet.hits().get(0)),
                resultSet.totalHits(),
