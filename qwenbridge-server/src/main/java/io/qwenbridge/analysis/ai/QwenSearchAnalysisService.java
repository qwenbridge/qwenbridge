...
public class QwenSearchAnalysisService implements SearchAnalysisService {
  ...
  private SearchAnalysis analyzeInternal(String query, String requestId) {
    ...
    cacheTraceHolder.set(
        AIAnalysisCacheTrace.miss(
            cacheKey.value(),
  ...
  private SearchAnalysis analyzeWithAI(String query, String requestId) {
    try {
      return CompletableFuture.supplyAsync(() -> analyzeWithAIBlocking(query, requestId))
      ...
      streamingEventPublisher.failed(requestId, "AI_ANALYSIS_FAILED", "AI analysis failed");
      return SearchAnalysis.fallback(query);
    }
  }
  ...
  private SearchAnalysis analyzeWithAIBlocking(String query, String requestId) {
    ...
    try {
      ...
      streamingEventPublisher.failed(
          requestId, "AI_STREAM_LIMIT_EXCEEDED", "AI streaming limit exceeded");
      return SearchAnalysis.fallback(query);
    } catch (Exception exception) {
      streamingEventPublisher.failed(requestId, "AI_STREAM_FAILED", "AI streaming analysis failed");
      return SearchAnalysis.fallback(query);
    }
  }
