...
public class DefaultThreatCorrelationEvaluator implements ThreatCorrelationEvaluator {
  ...
  public boolean matches(ThreatCorrelationRule rule, List<ThreatFinding> findings) {
    ...
    Set<ThreatType> detectedTypes =
        findings.stream().map(ThreatFinding::type).collect(Collectors.toSet());

    boolean allOfMatches =
        condition.allOf().isEmpty() || detectedTypes.containsAll(condition.allOf());

    boolean anyOfMatches =
        condition.anyOf().isEmpty() || condition.anyOf().stream().anyMatch(detectedTypes::contains);

    boolean noneOfMatches = condition.noneOf().stream().noneMatch(detectedTypes::contains);
