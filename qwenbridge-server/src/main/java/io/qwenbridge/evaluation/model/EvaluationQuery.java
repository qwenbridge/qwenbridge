...
public record EvaluationQuery(String id, String query, List<RelevanceLabel> labels) {
    ...
    return labels.stream()
        .collect(
            Collectors.toUnmodifiableMap(
                RelevanceLabel::documentId, Function.identity(), (first, second) -> first));
  }

  public long relevantDocumentCount() {
    return labels.stream().filter(RelevanceLabel::relevant).count();
  }
}
