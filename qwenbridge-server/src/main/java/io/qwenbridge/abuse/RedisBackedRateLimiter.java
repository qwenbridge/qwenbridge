...
public class RedisBackedRateLimiter implements RateLimiter {
    ...
    public RateLimitDecision consume(String policy, String subject, long limit, long cost) {
        ...
        try {
            ...
            return RateLimitDecision.allowed(policy, limit, limit - (value == null ? cost : value), Instant.ofEpochMilli(expiresAt));
        } catch (RuntimeException ex) {
            log.warn("Redis rate limiter unavailable; using {} fallback", properties.failOpenWhenRedisUnavailable() ? "fail-open" : "in-memory", ex);
