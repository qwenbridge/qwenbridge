...
public class PipelineEngine {
  ...
  public void execute(ExecutionContext context) {
    ...
    publish(
        PipelineEvents.pipelineStarted(
            snapshotFactory.create(context), metadata(context, sequence)));
    ...
    try {
      ...
      for (PipelineStep<?> step :
          steps.stream().sorted(Comparator.comparingInt(PipelineStep::order)).toList()) {

        if (context.stopped()) {

          context.addTrace(new PipelineTraceItem(step.name(), "SKIPPED", 0));
  ...
  private <T> void executeStep(
      PipelineStep<T> step, ExecutionContext context, AtomicLong sequence) {

    if (step.publishEvents()) {
      publish(
          PipelineEvents.stepStarted(
              step.stage(), snapshotFactory.create(context), metadata(context, sequence)));
    }
    ...
    try {
      ...
      context.addTrace(new PipelineTraceItem(step.name(), "EXECUTED", durationMs));
  ...
  private PipelineEventMetadata metadata(ExecutionContext context, AtomicLong sequence) {
    return PipelineEventMetadata.of(context.request().requestId(), sequence.incrementAndGet());
  }
