...
public final class PipelineEvents {
  ...
  public static PipelineEvent<PipelineContextSnapshot> pipelineStarted(
      PipelineContextSnapshot snapshot, PipelineEventMetadata metadata) {
  ...
  public static PipelineEvent<PipelineContextSnapshot> stepStarted(
      PipelineStage stage, PipelineContextSnapshot snapshot, PipelineEventMetadata metadata) {
  ...
  public static <T> PipelineEvent<T> started(PipelineStage stage, T payload) {
    return started(stage, payload, PipelineEventMetadata.empty());
  }

  public static <T> PipelineEvent<T> progress(PipelineStage stage, T payload) {
    return progress(stage, payload, PipelineEventMetadata.empty());
  }

  public static <T> PipelineEvent<T> completed(PipelineStage stage, T payload) {
    return completed(stage, payload, PipelineEventMetadata.empty());
  }

  public static <T> PipelineEvent<T> failed(PipelineStage stage, T payload) {
    return failed(stage, payload, PipelineEventMetadata.empty());
  }

  public static <T> PipelineEvent<T> info(PipelineStage stage, T payload) {
    return info(stage, payload, PipelineEventMetadata.empty());
  }

  public static <T> PipelineEvent<T> warning(PipelineStage stage, T payload) {
    return warning(stage, payload, PipelineEventMetadata.empty());
  }

  public static <T> PipelineEvent<T> started(
      PipelineStage stage, T payload, PipelineEventMetadata metadata) {
  ...
  public static <T> PipelineEvent<T> stopped(PipelineStage stage, T payload) {
  ...
  private static <T> PipelineEvent<T> build(
      PipelineStage stage, PipelineEventType type, T payload, PipelineEventMetadata metadata) {
    Class<T> payloadType =
        payload == null ? (Class<T>) Object.class : (Class<T>) payload.getClass();

    return new PipelineEvent<>(null, null, stage, type, metadata, payloadType, payload);
  }
}
