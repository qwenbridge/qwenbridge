...
final class StreamingPayloadMapper {
  ...
  TypedStreamingEvent map(StreamingEvent event) {
    ...
    try {
      return new TypedStreamingEvent(eventName, deserialize(eventName, rawData), rawData);
    } catch (JsonProcessingException ex) {
      return new TypedStreamingEvent(eventName, new UnknownStreamingPayload(rawData), rawData);
    }
  }
  ...
  private StreamingPayload deserialize(String eventName, String rawData)
    ...
    return switch (eventName) {
      case "stream.connected" -> objectMapper.readValue(rawData, ConnectedStreamingPayload.class);
      case "ai.token" -> objectMapper.readValue(rawData, AITokenStreamingPayload.class);
      case "ai.completed" -> objectMapper.readValue(rawData, AICompletedStreamingPayload.class);
      case "ai.failed" -> objectMapper.readValue(rawData, AIFailedStreamingPayload.class);
