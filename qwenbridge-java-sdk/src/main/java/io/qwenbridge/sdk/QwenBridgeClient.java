...
public class QwenBridgeClient {
  ...
  QwenBridgeClient(
      QwenBridgeClientConfig config, HttpClient httpClient, ObjectMapper objectMapper) {
    ...
    for (int attempt = 1; attempt <= config.retryPolicy().maxAttempts(); attempt++) {
      try {
        HttpResponse<String> response =
            httpClient.send(buildAnalyzeRequest(request), HttpResponse.BodyHandlers.ofString());
  ...
  private CompletableFuture<SearchAnalyzeResponse> analyzeAsync(
      SearchAnalyzeRequest request, int attempt) {
    return httpClient
        .sendAsync(buildAnalyzeRequest(request), HttpResponse.BodyHandlers.ofString())
        .handle(
            (response, throwable) -> {
              if (throwable != null) {
              ...
              try {
                return CompletableFuture.completedFuture(handleAnalyzeResponse(response));
              } catch (QwenBridgeApiException | QwenBridgeTransportException e) {
  ...
  private CompletableFuture<SearchAnalyzeResponse> asyncFailure(
      SearchAnalyzeRequest request, int attempt, RuntimeException failure) {
  ...
  private boolean shouldRetry(Throwable throwable, int attempt) {
    return attempt < config.retryPolicy().maxAttempts() && RetryClassifier.isRetryable(throwable);
  }

  private void sleepBeforeRetry(int attempt) {
    try {
      Thread.sleep(ExponentialBackoff.delayForAttempt(config.retryPolicy(), attempt).toMillis());
    } catch (InterruptedException e) {
      Thread.currentThread().interrupt();
      throw new QwenBridgeTransportException("QwenBridge retry sleep was interrupted", e);
    }
  }
