...
class QwenBridgeStreamingClientTest {
  ...
  void shouldConsumeConnectedSseEvent() throws Exception {
    ...
    assertEquals(
        "{\"requestId\":\"req-stream\",\"sessionId\":\"session-1\"}", events.getFirst().data());
  }

  @Test
  void shouldRejectBlankRequestId() {
    QwenBridgeStreamingClient client =
        new QwenBridgeStreamingClient(QwenBridgeClientConfig.localDefault());

    IllegalArgumentException exception =
        assertThrows(IllegalArgumentException.class, () -> client.stream(" ", event -> {}));
  ...
  void shouldFailWhenStreamReturnsNonSuccessStatus() throws Exception {
    ...
    RuntimeException exception =
        assertThrows(RuntimeException.class, () -> client.stream("req-stream", event -> {}).join());
  ...
  private HttpServer startSseServer(String body, int status) throws IOException {
    HttpServer httpServer = HttpServer.create(new InetSocketAddress(0), 0);
    httpServer.createContext(
        "/api/v1/search/stream/req-stream",
        exchange -> {
