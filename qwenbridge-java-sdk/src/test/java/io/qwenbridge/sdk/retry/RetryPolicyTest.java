...
class RetryPolicyTest {
  ...
  void shouldRejectMaxAttemptsBelowOne() {
    IllegalArgumentException exception =
        assertThrows(
            ...
            () -> new RetryPolicy(0, Duration.ofMillis(100), Duration.ofSeconds(1)));
  ...
  void shouldRejectNonPositiveMaxBackoff() {
    IllegalArgumentException exception =
        assertThrows(
            IllegalArgumentException.class,
            () -> new RetryPolicy(3, Duration.ofMillis(100), Duration.ZERO));
  ...
  void shouldRejectMaxBackoffLowerThanInitialBackoff() {
    IllegalArgumentException exception =
        assertThrows(
            IllegalArgumentException.class,
            () -> new RetryPolicy(3, Duration.ofSeconds(2), Duration.ofMillis(100)));

    assertEquals(
        "maxBackoff must be greater than or equal to initialBackoff", exception.getMessage());
  }
}
